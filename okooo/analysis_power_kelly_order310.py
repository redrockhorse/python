...
sqlstr=" select pdate,lg,homesxname,awaysxname,win,draw,lost,ypbegin,ypend,order310 from tb_rate_power_rs where gametime>'2017-05-14 11:00:00'  and resulenum is null and jishi_m is not null and lg is not null and win is not null order by gametime asc;"
...
for vals in result:
    ...
    if win>lost:
        ...
        usql ="  select a.r310,count(*) from  (select case  when resulenum>0 then '3'  when resulenum=0 then '1' when resulenum<0 then '0' end r310 from tb_rate_power_rs  where lg like  '"+lglike+"'  and resulenum is not null  and lost>"+str(uMin)+" and lost<"+str(uMax)+" and order310='"+order310+"') a group by a.r310;"
    else:
        uMax=float(win)+0.05
        uMin=float(win)-0.05
        usql ="  select a.r310,count(*) from  (select case  when resulenum>0 then '3'  when resulenum=0 then '1' when resulenum<0 then '0' end r310 from tb_rate_power_rs  where lg like  '"+lglike+"'  and resulenum is not null  and win>"+str(uMin)+" and win<"+str(uMax)+" and order310='"+order310+"') a group by a.r310;"
