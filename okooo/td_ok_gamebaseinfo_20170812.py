...
def getGameBaseInfo(okid,morder):
    ...
    try:
        ...
        print('there is an error ~~~:',str(Exception))

    hnamediv = tree.xpath(u'//div[@class="qpai_zi"]')
    if len(hnamediv) < 1:
        print('okid:'+okid+'_error_have_no_data hnamediv')
        return
    hmname = hnamediv[0].text
    anamediv = tree.xpath(u'//div[@class="qpai_zi_1"]')
    if len(anamediv) < 1:
        print('okid:'+okid+'_error_have_no_data anamediv')
        return
    ayname = anamediv[0].text

    shuju_div = tree.xpath(u'//div[@class="shuj"]')
    if len(shuju_div) < 1:
        print('okid:'+okid+'_error_have_no_data shuju_div')
        return
    ...
    print(id,okid,gametime,lg,hmname,ayname,season,gamerule,round,field,weather,temperature,h_t_w,h_t_d,h_t_l,h_t_s,h_t_o,h_h_w,h_h_d,h_h_l,h_h_s,h_h_o,a_t_w,a_t_d,a_t_l,a_t_s,a_t_o,a_a_w,a_a_d,a_a_l,a_a_s,a_a_o,hscore,ascore,h_hs,h_as)
    ...
    cur.execute(sql,(id,okid,gametime,lg,hmname,ayname,season,gamerule,round,field,weather,temperature,h_t_w,h_t_d,h_t_l,h_t_s,h_t_o,h_h_w,h_h_d,h_h_l,h_h_s,h_h_o,a_t_w,a_t_d,a_t_l,a_t_s,a_t_o,a_a_w,a_a_d,a_a_l,a_a_s,a_a_o,hscore,ascore,h_hs,h_as))
    conn.commit()
    ...
    if(len(results)>0):
        if(len(results[0])>0):
            ...
            if(len(row)>0):
                ...
                #mindate = datetime.datetime.strptime('2017-12-10', "%Y-%m-%d")
                if mindate != None:
                    for i in range(365):
