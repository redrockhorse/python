...
sqlstr=" select pdate,lg,homesxname,awaysxname,win_rq,draw_rq,lost_rq,ypbegin,ypend,rorder310 from tb_rate_power_rs where gametime>'2017-05-05 21:00:00'  and resulenum is null and jishi_m is not null and lg is not null  order by gametime asc;"
...
for vals in result:
    ...
    for c in lg:
        lglike+=c+'%'
    ysql = "select a.r310,count(*) from  (select case when resulenum+rq>0 then '3' when resulenum+rq=0 then '1'  when resulenum+rq<0 then '0' end r310 from tb_rate_power_rs  where lg like   '"+lglike+"' and resulenum is not null and ypbegin="+str(ypbegin)+" and ypend="+str(ypend)+" and rorder310='"+order310+"') a group by a.r310;"

    if win>lost:
        uMax=float(lost)+0.05
        uMin=float(lost)-0.05
        usql ="  select a.r310,count(*) from  (select case  when resulenum+rq>0 then '3'  when resulenum+rq=0 then '1' when resulenum+rq<0 then '0' end r310 from tb_rate_power_rs  where lg like  '"+lglike+"'  and resulenum is not null  and lost_rq>"+str(uMin)+" and lost_rq<"+str(uMax)+" and rorder310='"+order310+"') a group by a.r310;"
    else:
        uMax=float(win)+0.05
        uMin=float(win)-0.05
        usql ="  select a.r310,count(*) from  (select case  when resulenum+rq>0 then '3'  when resulenum+rq=0 then '1' when resulenum+rq<0 then '0' end r310 from tb_rate_power_rs  where lg like  '"+lglike+"'  and resulenum is not null  and win_rq>"+str(uMin)+" and win_rq<"+str(uMax)+" and rorder310='"+order310+"') a group by a.r310;"
    ...
    print '-----------------------------------------'
fliename='kelly_r'+time.strftime('%Y-%m-%d')
