...
def inference(data,w1,b1,w2,b2,wsl,bsl):
    ...
    with tf.name_scope("softmax_linear"):
        ...
        logits = tf.matmul(hidden2,wsl)+bsl
    return logits
...
def run_training():
    ...
    #predict class
        ...
        predict_logits = tf.matmul(hidden_layer2, wsl_c) + bsl_c
        ...
        with open(output_graph_path, "rb") as f:
            ...
            with tf.Session() as sess:
                ...
                for i in range(218):
                    ...
                    #print("预测结果:",rs)
                    #print("实际结果:",lt)
                    #ec = tf.nn.in_top_k(rs, lt, 1)
                    eq = sess.run(tf.equal(zv,rs))
                    #tf.assert_equal(x=zv,y=rs)
                    #print(rs)
                    #flag = sess.run(tf.argmax(tf.cast(eq, tf.int32),1))
                    #print(flag)
...
def predict():
    prefile = "E:\\ctfo\\tensorflow\\gambling\\data\\test_20170912.xls"
