      ...
      for line in f:
        ...
        for w in tokens:
          ...
          if word in vocab:
            ...
            vocab[word] = 1
      for w in vocab:
        vocab[w]=len(w)*100000+vocab[w]
      vocab_list = _START_VOCAB + sorted(vocab, key=vocab.get, reverse=True)
      #vocab_list = _START_VOCAB + sorted(vocab, key=len(vocab.items), reverse=True)
...
def test():
  #sentence="京藏高速 清河出口 五百米，有 两车 刮蹭事故，后车 小心"
  sentence='东二环 主路,外环方向,东便门桥 上,东便门桥 上,有 公交 与 轿车 事故,.'
  line = bytes(sentence, encoding = "gbk")
  print(line)
  tokenizer = None
