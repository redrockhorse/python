...
from headline import SmallConfig
...
def decode():
  ...
  with tf.Session() as sess:
    ...
    #vocab, rev_vocab = data_utils.initialize_vocabulary(rvocab_path)
    ...
    while sentence:
      ...
      #sentence = '东四环 慈云寺桥 上 南向北 最 内侧 车道 有辆 故障车 ， 后 车 小心 避让 。'
      #token_ids = data_utils.sentence_to_token_ids(tf.compat.as_bytes(sentence), vocab)
      token_ids = data_utils.sentence_to_token_ids(bytes(sentence, encoding = "gbk"), vocab)
      ...
      _, loss, output_logits_batch = model.step(sess, encoder_inputs, decoder_inputs, target_weights, bucket_id, True)
      print(loss)
