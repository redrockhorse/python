...
#使用时需要修改的部分
weather_kml='E:\\ctfo\\tensorflow\\palm\\data\\wr\\2017070816_prec.kml'
link_mid = 'E:\\ctfo\\tensorflow\\palm\\data\\wr\\map_link.MID'
link_mif = 'E:\\ctfo\\tensorflow\\palm\\data\\wr\\map_link.MIF'
southWest=[97.060000,24.130000]
northEast=[113.780000,34.450000]
...
def create_map_file_by_kml(kml_file):
    ...
    for pm in placemarks:
        ...
        if visibility[0] != '-100' or visibility[1]!='20':
            ...
            mif_file.write(path)
            midline=str(i)+',"'+rname.childNodes[0].data+'_'+str(i)+'",'+str(i)+',0,'+visibility[0]+','+visibility[1]+',2\n'
            mid_file.write(midline)
    mid_file.close()
    mif_file.close()

#从mid,mif文件中生成网格索引文件,这里要调用两次一次是从天气区域文件生成索引，一次是路链生成索引
def createIndexFileByMap(mid_path, mif_path, link_id_index, df_index, indexer_path,southWest,northEast,grid_len,link_type):
...
if __name__ == "__main__":
    ...
    if os.path.exists(weather_kml.replace('.kml','')+'_indexer_fill.txt'):
        ...
        createIndexFileByMap(weather_kml.replace('.kml','.mid'), weather_kml.replace('.kml','.mif'), 2, 6, weather_kml.replace('.kml','')+'_indexer.txt',southWest,northEast,0.01,2)
        #修改索引文件格式
        change_weather_format(weather_kml.replace('.kml','')+'_indexer.txt',weather_kml.replace('.kml','')+'_indexer_1.txt')
        #天气影响区域网格填充
        fill_weather_blank_grids(weather_kml.replace('.kml','')+'_indexer_1.txt',weather_kml.replace('.kml','')+'_indexer_fill.txt')

    #路网文件生成索引，最好判断一下，如果已经存在则不需要重复生成
    if os.path.exists(link_mid.replace('.MID','')+'_indexer.txt'):
        pass
    else:
        createIndexFileByMap(link_mid, link_mif, 0, 5, link_mid.replace('.MID','')+'_indexer.txt',southWest,northEast,0.01,2)

    # 读取路网索引文件.
    dict_indexs = read_map_indexer(link_mid.replace('.MID','')+'_indexer.txt')
    # 生成区域包含路链文件
    get_links_by_weather_pline(weather_kml.replace('.kml','')+'_indexer_fill.txt', weather_kml.replace('.kml','')+'link_by_weather.txt', dict_indexs)
    # 读取weatherinfo.
