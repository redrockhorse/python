    ...
    for i in range(0,24):
        ...
        for j in range(1,13):
            ...
            if n>=100:
                timeId = str(n)
            # print(timeId)
            zipfilename = '/Users/hongyanma/Downloads/pb/42/20200617/20200617_'+timeId+'.zip'
            pdZipFileDeCode(zipfilename, hour,hourData)
        print(hourData)
        rs = {}
        rs['data'] = []
        for xy in hourData:
            rs['data'].append(xy.split('_'))
        with open('/Users/hongyanma/Downloads/pb/wuhan/wh_'+hour+'.json','w') as outf:
            json.dump(rs,outf)
