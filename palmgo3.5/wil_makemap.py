...
import time
import datetime
import GpsUtils.grid_supporter
...
region_mif = "./data/weatherregion/reg_2017070815_prec.mif"
region_mid = "./data/weatherregion/reg_2017070815_prec.mid"
wl_mif = "./data/weatherregion/2017070815_prec.mif"
wl_mid = "./data/weatherregion/2017070815_prec.mid"
...
def createRegionFileByWeatherFile_1(f):
    ...
    for pm in placemarks:
        ...
        if visibility[0] != '-100' or visibility[1]!='20':
            ...
            print(path)
            #print(pointnum)
            midline=midline+'\n'+str(i)+',"'+rname.childNodes[0].data+'_'+str(i)+'",'+str(i)+',0,'+visibility[0]+','+visibility[1]
    region_mid_file.write(midline)
    region_mid_file.close()
    mifline=region_mif_header+mifline
    region_mif_file.write(mifline)
...
class WeatherRegion:
    ...
    path = []
    path_length=0
    def __init__(self,name,path,path_length):
        self.name=name
        self.path=path
    ...
    def isLinkIn(self,link):
        ...
        if self.isPointInRect(hp) or self.isPointInRect(mp) or self.isPointInRect(tp):
            ...
            print(mp)
            print(tp)
            print(self.getSouthWest())
            print(self.getNorthEast())
            print('******************************************8')
            '''
        else:
            return  False
...
def getRegionFromWeatherFile(weatherFile):
    ...
    for pm in placemarks:
        ...
        if visibility[0] != '-100' or visibility[1]!='20':
            ...
            region=WeatherRegion(rname.childNodes[0].data,bound,pointnum)
            #region.isLinkIn([])
