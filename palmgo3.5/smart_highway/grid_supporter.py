...
import midmif_supporter as msup
    ...
    if grid_cx >= 0 and grid_cx <= max_grid_cx and grid_cy >= 0 and grid_cy <= max_grid_cy:
        ...
        if updir == 1:
            ...
            if link_id_type == 1:
                ...
                key1 = linkid + ':1:' + '{:d}'.format(sublink_up_index)
                key2 = linkid + ':0:' + '{:d}'.format(sublink_down_index)
            elif link_id_type == 4:
                key1 = linkid + '1'
                key2 = linkid + '0'
...
# 返回对象，不再打印文件. (输入为link数组).
def return_grid_map2(dict_links):
    global dict_links_by_grid

    for link_id in sorted(dict_links.keys()):
        link = dict_links[link_id]

        for i in range(len(link.node_coord_list) - 1):
            bgn_coord_str = link.node_coord_list[i]
            end_coord_str = link.node_coord_list[i + 1]
            ...
            get_grid_of_sublink(link_id, 2, i, len(link.node_coord_list) - 2 - i, long1, lat1, long2, lat2)
