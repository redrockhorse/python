...
history = model.fit(x_train, train_y, batch_size=64, epochs=57, validation_split=0.1)
...
redball = np.argsort(result[0][0:33])
print(redball)
luckarr = ""
for i in range(6):
    print(redball[33-i-1]+1)
    luckarr += str(redball[33-i-1]+1) + ","

blueball = np.argsort(result[0][33:49])
print(blueball)
bluenum = 0
for i in range(1):
    print(blueball[16-i-1]+1)
    bluenum = blueball[16-i-1]+1
