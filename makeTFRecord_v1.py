...
if __name__ == '__main__':
    ...
    with tf.Graph().as_default():
        ...
        sess.close()
    '''
    dataFile = "E:\\ctfo\\tensorflow\\gambling\\data\\tb_pr_20170831.xls"
    #makeTFrecord(dataFile, 1886,30885, save_dir, 'train_v1')
    makeTFrecord(dataFile, 1,218, save_dir, 'ctest_v1')
   # makeTFrecord(dataFile, 30888,31887, save_dir, 'test_v1')
    ...
    #namedict = getOneHotCodeDic(namefile)
    #homenameHotCode =  getOneHotCode('飓风',namedict)
    #print(len(homenameHotCode))
    '''
    sess = tf.Session()
    data,labels = inputs(save_dir+'test_v1.tfrecord', batch_size=1,num_epochs=None)
    coord = tf.train.Coordinator()
    threads = tf.train.start_queue_runners(sess=sess, coord=coord)
    print(sess.run(data))
    coord.request_stop()
